...
if(i.documentMode&&i.documentMode!=l)l=i.documentMode!=8?7:8}if(M=Y&&l>=1||t&&l>=6||p&&l>=9.5||A&&l>=312){for(s=1;s<B;s++)v.push("");v.push(q());if(t&&c.hash!=q())c.hash="#"+G(q(),f);if(p)history.navigationMode="compatible";if(x&&Z!=-1){x=x.substr(Z+1).split("&");for(s=0;s<x.length;s++){p=x[s].split("=");if(/^(history|strict)$/.test(p[0]))k[p[0]]=isNaN(p[1])?/^(true|yes)$/i.test(p[1]):parseInt(p[1],10)!==0;if(/^tracker$/.test(p[0]))k[p[0]]=p[1]}}if(i.readyState=="complete")L();else{window==r&&d(L);
d(r).load(L)}}else if(!M&&c.href.indexOf("#")!=-1||A&&l<418&&c.href.indexOf("#")!=-1&&c.search!=""){i.open();i.write('<html><head><meta http-equiv="refresh" content="0;url='+c.href.substr(0,c.href.indexOf("#"))+'" /></head></html>');i.close()}else V();return{init:function(a,b){return u("init",a,b)},change:function(a,b){return u("change",a,b)},internalChange:function(a,b){return u("internalChange",a,b)},externalChange:function(a,b){return u("externalChange",a,b)},baseURL:function(){var a=c.href;if(a.indexOf("#")!=
-1)a=a.substr(0,a.indexOf("#"));if(a.substr(a.length-1)=="/")a=a.substr(0,a.length-1);return a},strict:function(a){if(a!==undefined){k.strict=a;return this}return k.strict},autoUpdate:function(a){if(a!==undefined){k.autoUpdate=a;return this}return k.autoUpdate},update:function(){J=f;this.value(g);J=m;return this},history:function(a){if(a!==undefined){k.history=a;return this}return k.history},tracker:function(a){if(a!==undefined){k.tracker=a;return this}return k.tracker},title:function(a){if(a!==undefined){a=
F(a);C(function(){ba=i.title=a;if($&&j&&j.contentWindow&&j.contentWindow.document){j.contentWindow.document.title=a;$=m}if(!N&&Y)c.replace(c.href.indexOf("#")!=-1?c.href:c.href+"#");N=m},50,f);return this}return i.title},value:function(a){if(a!==undefined){a=R(F(S(a,f)));if(a=="/")a="";if(g==a&&!J)return;N=f;g=a;if(k.autoUpdate||J){H=f;E(f);v[D.length]=g;if(A)if(k.history){c[o][c.pathname]=v.toString();B=D.length+1;if(l<418){if(c.search==""){K.action="#"+g;K.submit()}}else if(l<523||g==""){a=i.createEvent("MouseEvents");
a.initEvent("click",f,f);var b=i.createElement("a");b.href="#"+g;b.dispatchEvent(a)}else c.hash="#"+g}else c.replace("#"+g);else if(g!=q())if(k.history)c.hash="#"+G(g,f);else c.replace("#"+g);t&&l<8&&k.history&&C(W,50,f);if(A)C(function(){H=m},1,f);else H=m}return this}if(!M)return null;return F(S(G(g,m),m))},path:function(a){if(a!==undefined){var b=this.queryString();this.value(a+(b?"?"+b:""));return this}a=this.value();return a.indexOf("?")!=-1?a.split("?")[0]:a},queryString:function(a){if(a!==
...
if(e!=-1){b=b.substr(e+1);n=b.split("&");b=[];for(e=0;e<n.length;e++){h=n[e].split("=");h[0]==a&&b.push(h[1])}if(b.length!==0)return b.length!=1?b:b[0]}},pathNames:function(){var a=this.path(),b=a.split("/");if(a.substr(0,1)=="/"||a.length===0)b.splice(0,1);a.substr(a.length-1,1)=="/"&&b.splice(b.length-1,1);return b},parameterNames:function(){var a=this.value(),b=a.indexOf("?"),h=[];if(b!=-1){a=a.substr(b+1);if(a!=""&&a.indexOf("=")!=-1){a=a.split("&");for(b=0;b<a.length;b++){var e=a[b].split("=")[0];
d.inArray(e,h)==-1&&h.push(e)}}}return h}}}();d.fn.address=function(z){var u=function(){var q=z?z.call(this):/address:/.test(d(this).attr("rel"))?d(this).attr("rel").split("address:")[1].split(" ")[0]:d(this).attr("href").replace(/^#/,"");d.address.value(q);return false};d(this).click(u).live("click",u)}})(jQuery);
